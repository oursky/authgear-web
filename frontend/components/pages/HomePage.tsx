...
const pageScripts: string[] = [
  // Disable first option in "how did you hear" dropdown
  `
  var selectionDropdown = document.getElementById("how-hear");
  if (selectionDropdown) {
    var selectionOptions = selectionDropdown.getElementsByTagName("option");
    if (selectionOptions.length > 0) selectionOptions[0].disabled = true;
  }
  `,
  // Autofill form fields from URL params
  `
  var urlParams = new URLSearchParams(window.location.search);
  if (urlParams.has('email')) {
    var emailEl = document.getElementById("Email-6");
    if (emailEl) emailEl.value = urlParams.get('email');
  }
  if (urlParams.has('name')) {
    var nameEl = document.getElementById("Name-5");
    if (nameEl) nameEl.value = urlParams.get('name');
  }
  `,
  // intlTelInput for phone field
  `
  ...
  if (input && typeof window.intlTelInput === 'function') {
    var iti = window.intlTelInput(input, {
      ...
      geoIpLookup: function(callback) {
        fetch("https://ipapi.co/json")
          ...
          .catch(function() { callback("hk"); });
      },
      preferredCountries: ["hk", "sg", "au"],
    ...
    var updateInputValue = function() {
      if (country) country.value = iti.getSelectedCountryData().name;
    };
    ...
    input.addEventListener('blur', function() {
      ...
      if (input.value.trim()) {
        if (iti.isValidNumber()) {
          ...
          if (errorMsg) { errorMsg.innerHTML = errorMap[errorCode]; errorMsg.classList.remove("hide"); }
        }
      }
    });
    input.addEventListener('change', resetField);
                ...
                <div className="_2-block-flex-image footer-form">
                  <div id="wf-form-Authgear-Talk-with-Us" className="form-block w-form">
                    <form id="wf-form-Authgear-Talk-with-Us-2" name="wf-form-Authgear-Talk-with-Us-2" data-name="Authgear Talk with Us" method="post" className="contact-form">
                      <div className="margin-vertical margin-small"><label htmlFor="Name-5" className="getdemo-label">{t('formFieldName')}<span className="text-span-7">*</span></label><input className="getdemo-field w-input" maxLength={256} name="Name" data-name="Name" placeholder="" type="text" id="Name-5" required /></div>
                      <div className="margin-vertical margin-small"><label htmlFor="Email-6" className="getdemo-label">{t('formFieldEmail')}<span className="text-span-8">*</span></label><input className="getdemo-field w-input" maxLength={256} name="Email" data-name="Email" placeholder="" type="email" id="Email-6" required /></div>
                      <div className="margin-vertical margin-small"><label htmlFor="Phone-3" className="getdemo-label">{t('formFieldPhone')}<span className="text-span-9">*</span></label><input className="getdemo-field w-input" maxLength={256} name="Phone" data-name="Phone" placeholder="" type="tel" id="Phone-3" required /><input className="text-field w-input" maxLength={256} name="Country" data-name="Country" placeholder="" type="text" id="Country-4" required />
                        <div className="html-embed w-embed"><span id="valid-msg" className="hide">Valid number</span>
                          <span id="error-msg" className="hide"></span>
                        </div>
                      </div>
                      <div className="margin-vertical margin-small"><label htmlFor="Company-5" className="getdemo-label">{t('formFieldCompany')}<span className="text-span-10">*</span></label><input className="getdemo-field w-input" maxLength={256} name="Company" data-name="Company" placeholder="" type="text" id="Company-5" required /></div>
                      ...
                      <div className="margin-vertical margin-medium"><input type="submit" data-wait={t('formSubmitWait')} className="getdemo-submit plausible-event-name--contact-form-submit w-button" value={t('formSubmitValue')} /></div>
                    </form>
