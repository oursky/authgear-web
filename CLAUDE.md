...
- `ja`, `es`, `de` are partial market-test locales: only the home page, `/pricing`, `/auth-toolkit`, `/schedule-demo` and `/tools/*` are translated (URL slugs stay English). The allowlist is `PARTIAL_LOCALES` / `isPartialLocalePath` in `src/lib/i18n.ts`; everything else links to and redirects (302, non-forced, `public/_redirects`) to the English page. See "Partial locales" in `docs/ARCHITECTURE-ASTRO.md`.
