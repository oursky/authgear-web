...
- **Relative internal links** (`href="/post/nextjs-jwt-authentication"`, `href="/features/passkeys"`, etc.) are preserved verbatim in the markdown. `/post/*` links resolve via the `/post/{slug}` → `/blog/{slug}` 301 redirect added on the Astro site; other relative paths must already exist as Astro routes.
- **Absolute self-domain links** (`https://www.authgear.com/post/{slug}`, `https://www.authgear.com/features/...`, etc.) are **rewritten by the migration script** to site-relative paths (`/post/{slug}`, `/features/...`). This keeps internal links working regardless of which platform serves `www.authgear.com` at any given moment — they no longer depend on a cross-platform redirect being in place. The rewrite rule: any absolute URL whose origin is `https?://(www\.)?authgear\.com` has its origin stripped.
- **Link audit step**: after writing all markdown, the migration script walks every post's links and writes `/tmp/blog-link-audit.txt`. Each link is classified as (a) resolves to a known Astro route, (b) `/post/*` covered by the redirect, (c) external, or (d) potential dead link. Class (d) is flagged for human review, not auto-fixed.
