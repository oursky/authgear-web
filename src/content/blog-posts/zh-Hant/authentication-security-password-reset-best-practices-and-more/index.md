      ...
      <tr>
        <td>
          <ul>
            ...
            <li>不要忘記設定 Access Token 的過期時間。</li>
            <li>不要對秘密或 Token 進行硬編碼。</li>
            <li>不要記錄敏感資訊。</li>
...
識別密碼重設會話的常見方法是在 URL 中包含 Token。然而，在該 Token 中使用個人識別資訊 (PII) 會帶來嚴重的安全風險。即使加密，也總是存在洩漏的可能性，可能會暴露敏感的用戶資料。

我們遇到過使用加密的用戶 ID 作為重置 Token 的情況，這是非常不安全的。此外，不正確的加密或雜湊技術（例如使用 MD5 等弱演算法）可能會進一步加劇問題。

為了保護使用者隱私和安全，請務必為每個密碼重設工作階段產生隨機的非 PII Token。對這些 Token 實施嚴格的過期限制，並採用速率限制措施來阻止暴力攻擊。
...
### 不要忘記設定 Access Token 的有效期

許多身份驗證系統中的一個關鍵監督是 Access Token 沒有過期時間。如果沒有定義的生命週期，受損的 Token 可以授予攻擊者對使用者帳號的無限期存取權限（[Token 端劫持](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html#token-sidejacking)）。

想像攻擊者獲得了有效的 Access Token。由於沒有過期時間，他們可以無限期地利用該 Token 進行惡意目的。這種情況比您想像的更常見 - 您的瀏覽器可能儲存多個 Access Token，為攻擊者提供了潛在的目標。

即使使用 HTTPS 加密，沒有過期日期的 Access Token 仍然容易受到攻擊。隨著時間的推移，用戶設備的違規或不斷演變的安全威脅可能會損害 Token 的安全性。

為了減輕這些風險，請對 Access Token 實施嚴格的過期策略。較短的生命週期（例如一小時）可以顯著減少受損 Token 造成的潛在損害。攻擊者需要在有限的時間內攔截並使用 Token，這使得他們的任務變得更加困難。

### 不要對秘密或 Token 進行硬編碼

在按時完成任務的壓力下，開發人員經常採取捷徑，將秘密直接硬編碼到程式碼中是一個常見的陷阱。這種做法會帶來「災難性」的安全風險。

我們親眼目睹了這個錯誤的後果。在一種情況下，JWT 被硬編碼到函數中，導致整個系統容易受到攻擊。如果您的原始程式碼被揭露——無論是透過受感染的版本控制伺服器還是意外洩漏——任何硬編碼的秘密都會立即受到損害，從而使您的安全措施毫無用處。

為了保護您的應用程序，請遵守以下基本原則：

- **避免硬編碼：**切勿將機密或 Token 直接嵌入程式碼中。
...
成功重設密碼後，提醒使用者查看帳戶上的所有活動登入工作階段。更好的是，在密碼重置後自動使所有現有會話失效，以提高安全性。這確保了即使攻擊者透過被盜的會話 Token 進行訪問，一旦更改密碼，他們也會被註銷，從而切斷他們對帳戶的存取權限。
...
一次性 Token（通常用於無密碼登入連結或多因素身份驗證重設）可以改善使用者體驗，但實施不當可能會使它們無效。確保您的系統正確追蹤並使這些 Token 失效：

- **唯一識別：** 為每個發行的一次性 Token 分配一個唯一識別碼。
- **已使用 Token 追蹤：**維護已使用 Token 的清單（或資料庫記錄）。
- **使用時驗證：** 在根據一次性 Token 授予存取權限之前，請先驗證該 Token 是否尚未使用且尚未過期。

透過實施這些措施，您可以防止攻擊者重複使用 Token 或利用過期 Token 來取得存取權限。這將創建更安全且用戶友好的密碼重置（或無密碼登入）體驗。
...
不可以。切勿記錄密碼、Token、API 金鑰或其他機密。編輯和屏蔽日誌中的敏感欄位。
