...
API 閘道充當看門人，在授予對後端服務的存取權限之前要求提供身分證明。此證明可以有多種形式，例如 API 金鑰、OAuth Token 或基本驗證憑證。
...
- **可擴充性：** 有效處理不同的流量負載。
- **範例：** 驗證 JWT、從資料庫存取使用者資訊或實施複雜的授權規則。
...
讓我們考慮一個常見的場景：需要存取受保護的 API 的行動應用程式。為了確保此互動的安全，我們可以採用 **JSON Web Token (JWT)** 作為驗證機制。

### 工作原理：

1. **使用者登入：** 行動應用程式提示使用者使用其憑證登入。
1. **Token 產生：** 身份驗證成功後，身份驗證伺服器會發出包含使用者聲明（例如使用者 ID、角色、權限）的 JWT。
1. **API 請求：** 行動應用程式將 JWT 包含在後續 API 請求的授權標頭中。
1. **Token 驗證：** API網關攔截請求並使用公鑰驗證JWT。
...
1. **身份驗證檢查：** API網關攔截請求並檢查客戶端提供的身份驗證憑證。這可能涉及檢查 API 金鑰、JWT 或其他形式的身份驗證。
...
- **Token 管理：** Access Token 的處理和驗證（如果使用）。
- **安全措施：** 加密、Token 簽署和其他安全實踐來保護敏感資料。
...
- **使用基於 Token 的身份驗證：** JWT 或 OAuth Token 可用於承載使用者聲明和權限。 ****
- **驗證使用者聲明：**驗證 Token 中使用者資訊的真實性和完整性。
...
授權伺服器專門設計用於在身份驗證成功後向客戶端頒發 Access Token。它確定客戶端應用程式是否有權存取特定資源。
