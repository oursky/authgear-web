...
excerpt: "以實務角度拆解 OIDC 規範，說明流程、Token、宣告、範圍與端點。了解開發者真正需要的重點，實作更安全的驗證機制。"
coverImage: ./cover.webp
category: engineering
featured: false
metaTitle: "OIDC 規範解析：開發者需要掌握的關鍵概念"
metaDescription: "以實務角度拆解 OIDC 規範，說明流程、Token、宣告、範圍與端點。了解開發者真正需要的重點，實作更安全的驗證機制。"
...
本文會把 OIDC 規範拆解成開發者真正需要掌握的關鍵概念。我們不會逐字重述整份規範，而是聚焦在 OIDC 的實務運作：參與角色、交換的 Token、使用的流程，以及維持驗證可靠與安全的核心機制。
...
OpenID Provider 是負責驗證使用者並簽發 Token 的身分服務。它會驗證憑證、套用安全政策，並產生經過密碼學簽章的身分聲明。
...
Token 是 OIDC 的核心。它們在各方之間傳遞驗證結果與授權資料。
...
Claims 是出現在 Token 中或由 UserInfo 端點回傳的鍵值資料。常見 Claims 包含：
...
驗證成功後，Client 會在 Token 端點以授權資料交換 Token。此步驟在伺服器對伺服器通道進行，可保護敏感憑證。
...
JSON Web Key Set（JWKS）端點提供驗證 Token 簽章所需的公開金鑰。Client 透過它安全驗證 ID Token。
...
它把使用者驗證與 Token 交換分離，降低 Token 洩漏風險。
...
收到 ID Token 不代表可以直接信任。應用必須驗證簽章、issuer、audience、過期時間與 nonce。若省略這些步驟，系統會暴露在重放攻擊與 Token 濫用風險中。

### **請求過多 Scopes**

過度請求 scopes 會暴露不必要的使用者資料，並違反隱私最佳實務。落實最小權限原則可降低資料暴露並更容易符合法規要求。

### **在 Client 端不安全儲存 Token**

把 Token 放在不安全的瀏覽器儲存位置會提升 XSS 攻擊暴露面。使用安全儲存機制與短效 Token 可降低被竊風險，整體安全性更高。
...
只要掌握角色、Token、流程、宣告與端點等核心概念，就能更有信心且更正確地實作 OIDC。這可同時降低安全風險、提升互通性，並讓驗證架構更具未來性。
