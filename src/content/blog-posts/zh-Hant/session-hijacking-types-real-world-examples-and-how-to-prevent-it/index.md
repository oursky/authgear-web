...
metaDescription: "會話劫持是攻擊者竊取合法使用者會話 Token、繞過 MFA 並取得未授權存取權的攻擊手法。本文帶你掌握類型、案例與防禦策略。"
publishedAt: 2024-12-29T06:24:37.185Z
updatedAt: 2026-02-28T12:53:04.971Z
draft: false
---

會話劫持（Session Hijacking）是一種網路攻擊，惡意攻擊者會透過竊取使用者的 Session ID 或會話 Token，接管已授權的網頁會話。一旦取得這個暫時性的「通行憑證」，攻擊者就能繞過登入畫面，甚至繞過多因素驗證（MFA），冒充使用者存取敏感資料或執行未授權交易。
...
會話劫持是指攻擊者在使用者已完成驗證後，接管其有效會話。當使用者登入網站或應用後，系統通常會簽發一個會話 Token，讓使用者在後續請求中無須反覆輸入帳號密碼。這個 Token 就像「數位鑰匙」。

若駭客成功竊取這把鑰匙，就能假冒合法使用者，未授權存取敏感資料、個資，甚至內部系統。雖然手法各異，但核心原理一致：攔截或竊取會話 Token，並在 Token 有效期內重放使用。由於攻擊發生在登入之後，往往更不易察覺，對企業與使用者都具有高度風險。
...
1. **封包嗅探（Packet Sniffing）**  
   這是最基礎的會話劫持方式之一。攻擊者利用網路監聽工具擷取使用者與伺服器間的未加密流量。若會話 Token 透過 HTTP 傳輸（而非 HTTPS），就很容易被攔截。公共 Wi-Fi 尤其危險，因為攻擊者可更輕易嗅探未保護封包。
1. **中間人攻擊（Man-in-the-Middle, MITM）**  
   MITM 指攻擊者在兩端通訊之間祕密攔截、甚至竄改資料，常見於使用者瀏覽器與伺服器之間。攻擊者可在傳輸途中取得會話 Token，進而接管會話。當網站未正確使用加密、或使用者連線到不安全網路時，MITM 攻擊成功率會提高。
1. **跨站腳本（Cross-Site Scripting, XSS）**  
   XSS 允許攻擊者把惡意腳本注入可信網站。當腳本在使用者瀏覽器執行時，可竊取 Session Cookie 或 Token 並回傳給攻擊者。XSS 常與社交工程並用，例如誘導使用者點擊惡意連結。
...
這些攻擊都在提醒同一件事：必須透過強加密、頻繁 Token 輪換與正確應用安全實務，才能有效保護使用者會話。
...
會話 Token 常存放於 Cookie，因此 Cookie 安全設定至關重要：
...
限制會話有效期間，並在閒置一段時間後自動登出，可縮短攻擊者可利用的時間窗口。可進一步採用滾動式會話（rolling sessions），定期刷新會話 Token。
...
MFA 可增加額外驗證層。即使攻擊者拿到會話 Token，仍可能需要突破第二驗證因素，顯著提高攻擊成本。

### 7. 限縮會話使用範圍

可將會話 Token 綁定到特定 IP 或 User-Agent。當同一 Token 出現在異常環境時，系統可立即失效該會話，阻止未授權使用。

### 8. 實作內容安全政策（CSP）

CSP 可限制頁面可載入的資源來源，降低惡意腳本執行機會，進而減少透過 XSS 竊取會話 Token 的風險。
...
**發生了什麼？**  
攻擊者利用了會話 Token 未妥善輪換／重生的缺陷。一旦取得有效 Cookie，便可重複使用。該事件最終估計影響 **30 億個 Yahoo 帳號**。

**關鍵教訓：**  
Token 輪換與到期失效策略不可缺。透過登入後重生與舊 Token 失效，可降低被重放利用的機率。

### 4. 加密貨幣交易所 MITM 攻擊

攻擊者曾透過 **MITM** 劫持加密貨幣交易平台會話。攔截會話 Token 後，可冒充使用者並轉移資產。
...
Facebook 在 2018 年揭露漏洞，攻擊者可竊取類似會話 Token 的 access token，影響近 **5,000 萬個帳號**。

**發生了什麼？**  
「View As」功能缺陷導致會話 Token 暴露，攻擊者可提取 Token 並冒充使用者。

**關鍵教訓：**  
Facebook 透過強制受影響使用者登出並重新驗證 Token 應對。此事件凸顯了 Token 生命週期管理與異常監控的重要性。

這些案例共同指出：會話 Token 若未被妥善保護（例如透過 HTTP 暴露、未輪換、加密不足），任何平台都可能遭受重大衝擊。
...
- **會話劫持**：攻擊者竊取或攔截有效會話 Token，接管已登入會話並冒充使用者。  
...
- **XSS**：注入惡意腳本，從瀏覽器中竊取 Cookie 或 Token。  
- **MITM**：攔截使用者與伺服器之間傳輸，擷取 Token。  
- **惡意軟體／Keylogger**：在端點側竊取 Cookie 或登入資料。  
- **會話固定**：迫使使用者用攻擊者指定的 Session ID 登入。

**最佳防線：** 落實 HTTPS、安全 Cookie（HttpOnly、Secure、SameSite）與登入後 Token 重生。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><b>核心防禦</b></td>
      <td>HTTPS、Cookie 安全旗標、異常偵測、Token 輪換。</td>
...
**關鍵結論：**  
會話劫持發生在**登入後**，會話固定發生在**登入前**。若要防止會話固定，應用必須在成功登入後一律簽發新的會話 Token。
