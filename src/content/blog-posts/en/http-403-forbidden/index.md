...
excerpt: "A 403 Forbidden error means the server understood your request but refuses to fulfil it. Here is what causes it and how to fix it on Nginx, Apache, Cloudflare, and AWS."
coverImage: ./cover.webp
category: engineering
featured: false
metaTitle: "HTTP 403 Forbidden: Fixes for Nginx, Cloudflare & AWS | Authgear"
metaDescription: "A 403 Forbidden error means the server refuses your request. Learn what causes it and how to fix it on Nginx, Apache, Cloudflare, and AWS S3."
...
# Note: <Directory> blocks are not valid in .htaccess — use bare directives
...
Either add an `index.html` to the directory, or selectively enable directory listing by placing this in the directory's `.htaccess`:

```apache
# .htaccess in the downloads directory
Options +Indexes
Require all granted
```

To scope this to a specific path, place it in the server config or vhost instead of `.htaccess`:
...
Cloudflare returns 403 when one of its security layers blocks the request. The HTTP status is 403, but Cloudflare's response body labels these blocks as "error 1020" — so users may report the error as "1020" while your logs show "403". They are the same incident.
...
For immediate debugging, you can temporarily disable "Under Attack Mode" (Dashboard → Security → Settings) and test from a fresh IP, or use Cloudflare's "Skip" action on a specific rule while you diagnose.
...
If you are hitting 403s caused by OAuth scope or role mismatches in your app, the [Authgear JWT Debugger](/tools/jwt-jwe-debugger) lets you inspect the token payload immediately. For a full breakdown of the 401 status code and how to handle token validation flows, see [HTTP 401 Unauthorized](/post/http-401-unauthorized). For the server-side picture of how proxies and gateways can cause related access errors, see [HTTP 502 Bad Gateway](/post/http-502-bad-gateway).
