...
metaDescription: "504 Gateway Timeout means your proxy waited too long for the upstream. Learn what causes it, how it differs from 502, and how to fix it on Nginx, Apache, Cloudflare, and AWS."
...
faq:
  ...
  - q: "Why does Cloudflare return a 504?"
    a: "Cloudflare returns a 504 (or its equivalent 524 timeout error) when your origin server does not respond within Cloudflare's edge timeout of approximately 100 seconds. On Free and Pro plans this timeout cannot be raised — the fix is to make your origin faster, or to refactor the slow operation into a background job and return a 202 Accepted response immediately."
  ...
  - q: "What causes a 504 on Vercel or Netlify?"
    a: "Serverless functions on Vercel's Hobby plan have a 10-second execution limit; Pro plan functions have a 60-second limit. On Netlify, the default function timeout is 10 seconds (26 seconds on paid plans). If your function exceeds these limits — due to a slow database query, an external API call, or a cold start — the platform terminates the function and returns a 504. Strategies: move slow work to background jobs, use streaming responses, or upgrade to a plan with a higher timeout."
...
A **504 Gateway Timeout** error means the server acting as a proxy or gateway forwarded a request upstream but never got a response back in time. The proxy — Nginx, Cloudflare, an AWS load balancer — was perfectly functional. What failed was the application server behind it, which took too long to reply.
...
- **Timely response** — within the proxy's configured timeout window. The upstream did not need to crash or return garbage; it just needed to take too long.
...
| Cloudflare | Edge timeout | ~100 seconds (not configurable below Enterprise) |
| AWS ALB | Target response timeout | 60 seconds |
| Vercel | Function execution (Hobby) | 10 seconds |
| Vercel | Function execution (Pro) | 60 seconds |
| Netlify | Function execution | 10 seconds default |
...
<VirtualHost *:443>
    ...
    # Time allowed for the backend to respond
    ProxyTimeout 60
...
### Fix: Cloudflare 504 Gateway Timeout

Cloudflare's edge timeout is approximately 100 seconds. On Free and Pro plans, this cannot be raised — only Enterprise customers can configure it higher.

If your origin takes longer than 100 seconds to respond:
...
For standard 504s where the origin is briefly slow, Cloudflare's error analytics (Security → Events) can tell you whether the 504s are clustered at a particular time or endpoint.
...
A 504 from ALB is almost always the target response timeout. To update it via the AWS CLI:

```bash
aws elbv2 modify-target-group \
  --target-group-arn arn:aws:elasticloadbalancing:us-east-1:123456789:targetgroup/my-tg/abc123 \
  --target-group-attributes Key=stickiness.enabled,Value=false \
  --attributes Key=load_balancing.algorithm.type,Value=round_robin
```

To change the response timeout specifically:

```bash
aws elbv2 modify-target-group-attributes \
  --target-group-arn arn:aws:elasticloadbalancing:us-east-1:123456789:targetgroup/my-tg/abc123 \
  --attributes Key=target_group.health_check.enabled,Value=true
```

The correct attribute key for the response timeout is set at target group level under **Advanced configuration** in the AWS Console (Attributes → Deregistration delay / Response timeout). The default is 60 seconds; the maximum is 3600 seconds.

Again: raising this is a workaround. Check your Target Response Time CloudWatch metric first to understand whether the p95/p99 latency genuinely warrants a higher limit.

### Fix: Vercel / Netlify Serverless 504

Serverless platforms enforce hard execution time limits:
...
| Netlify | All plans | 10 seconds default, 26 seconds max |
...
2. **Move to Edge Functions.** Edge functions have lower limits (1.5s–5s CPU time) but much lower latency — not a fit for slow compute, but good for auth checks and header manipulation.
3. **Use `waitUntil()` for background work.** On Vercel, `waitUntil(promise)` lets you continue work after the response is sent. Useful for analytics or logging, not for user-facing results.
4. **Move the slow operation to a proper background queue.** Return a job ID immediately; poll or use a webhook to surface results.
...
`CONCURRENTLY` in PostgreSQL builds the index without locking the table — safe for production.
...
- **Profile before raising.** Every time you are tempted to increase a timeout, run `EXPLAIN ANALYZE` first (or the equivalent for your database). Fix what you find, then set the timeout to reflect the optimised performance.
...
### Why does Cloudflare return a 504?

Cloudflare returns a 504 (or its own 524 error) when your origin does not respond within roughly 100 seconds. On Free and Pro plans this limit cannot be raised. The fix is to make your origin respond faster, or to return a `202 Accepted` immediately and move the slow work to a background queue.
...
Serverless functions on Vercel's Hobby plan have a 10-second execution limit; Pro plan functions have 60 seconds. Netlify functions default to 10 seconds. If a function exceeds these limits due to a slow database query, cold start, or external API call, the platform terminates it and returns a 504. Move slow operations to background jobs or upgrade to a higher-tier plan.
