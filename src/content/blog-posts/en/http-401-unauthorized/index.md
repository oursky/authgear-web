...
featured: false
metaTitle: "HTTP 401 Unauthorized: Causes & Fixes (OAuth, JWT, Basic Auth) | Authgear"
metaDescription: "401 Unauthorized means authentication is missing or invalid. Learn what causes it, how it differs from 403, and how to fix it for OAuth, JWT, Basic Auth, and API key flows."
...
RFC 9110 is explicit on this point: a 401 is appropriate when the request "lacks valid authentication credentials." A 403 is appropriate when "the server understood the request but refuses to fulfil it" regardless of authentication state.
...
A typo in the Authorization header format is more common than you think. The correct format is:

```http
Authorization: Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...
```

Common mistakes: `authorization: Bearer ...` (lowercase — some servers are case-sensitive on the header name), `Bearer: eyJ...` (wrong format), or a trailing space after "Bearer". Some servers are also case-sensitive to "Bearer" vs "bearer".
...
The token's `aud` (audience) claim must match what the resource server expects. If the access token was issued for `api.service-a.com` but the request hits `api.service-b.com`, the resource server must reject it — otherwise tokens are portable across services in ways the auth server did not intend.
...
The correct format is a single space between "Bearer" and the token, and the header name is case-insensitive by the HTTP spec (though some servers enforce case):
...
**Set short access token expiry.** The industry standard is 15 minutes for access tokens. A short expiry limits the blast radius if a token is leaked — an attacker has at most 15 minutes before the token is useless. Refresh tokens should be longer-lived (30–90 days) but single-use under rotation.
