export const passkeyDemo = {
  ...
  heroDescription:
    '使用 WebAuthn API 建立真正的通行密鑰，檢視解碼後的憑證內容，再用它登入，逐項觀察伺服器會執行的每個驗證步驟。所有運算都透過 WebCrypto 在你的瀏覽器本機完成。無需帳號，任何資料都不會離開你的裝置。',
  iframeTitle: '通行密鑰示範與 WebAuthn 測試工具',
  policyPrefix:
    '你的資料安全是我們的首要考量。通行密鑰的建立、憑證檢視與簽章驗證全部透過 WebAuthn 與 WebCrypto API 在瀏覽器內完成。這裡沒有任何後端伺服器，此工具也不會將任何資料傳送或儲存到你的裝置之外。原始碼請見：',
  ...
  step5Title: '像伺服器一樣驗證：',
  step5Body:
    '伺服器會執行的每項檢查都會以通過/失敗徽章呈現並附上說明：流程類型、challenge、來源、RP ID 雜湊、旗標，以及 WebCrypto 簽章驗證。',
  ...
  plat1Name: 'Apple',
  plat1Desc: 'iOS 16+ 與 macOS 13+。Face ID 或 Touch ID，透過 iCloud 鑰匙圈同步。',
  plat2Name: 'Android 與 Chrome',
  plat2Desc: 'Android 9+。指紋或螢幕鎖定，透過 Google 密碼管理員同步。',
  plat3Name: 'Windows',
  plat3Desc: 'Windows 10 與 11。Windows Hello 臉部辨識、指紋或 PIN。',
  plat4Name: '密碼管理員',
  ...
  faqWebauthnBody:
    'WebAuthn（Web Authentication）是通行密鑰背後的 W3C 標準瀏覽器 API。它不使用共享密碼，而是讓你的裝置為每個網站建立一組公私金鑰對：私鑰永遠不會離開你的驗證器，網站只儲存公開金鑰。登入是一次 challenge-response 簽章。瀏覽器會將每個憑證綁定到建立它的網域，這正是通行密鑰能防範網路釣魚的原因。',
  ...
  faqSafeBody:
    '安全。此頁面建立的是真正的通行密鑰，但它只對本網站有效，除了這個示範之外沒有任何用途。私鑰保存在你裝置的驗證器中；公開金鑰與憑證中繼資料只存在你瀏覽器的 localStorage。沒有伺服器，因此任何資料都不會被傳送。你可以一鍵刪除示範紀錄，並隨時從裝置中移除通行密鑰本身。',
  faqDeleteTitle: '如何從裝置中刪除示範通行密鑰？',
  faqDeleteIntro: '工具中的「Forget」只會移除此頁面的紀錄。要從裝置中移除通行密鑰本身：',
